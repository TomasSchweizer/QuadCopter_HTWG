...
uint16_t usIdleTaskStackSize = tskIDLE_STACK_SIZE;
	...
	xReturn = xTaskGenericCreate( prvIdleTask, "IdleTask", usIdleTaskStackSize, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle, pxIdleTaskStackBuffer, pxIdleTaskTCBBuffer, NULL ); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
	HIDE_Workload_StoreTaskHandle(priority_NUM_IDLE_TASK,xIdleTaskHandle);
...
/*-----------------------------------------------------------*/
	...
	{
		return uxTaskGetTaskNumber( (TaskHandle_t) pxCurrentTCB );
	}
