...
uint16_t usTimerTaskStackSize = configTIMER_TASK_STACK_DEPTH;
	...
	if( xTimerQueue != NULL )
	{
		...
		#if ( INCLUDE_xTimerGetTimerDaemonTaskHandle == 1 )
		{
			...
			xReturn = xTaskGenericCreate( prvTimerTask, conficTIMER_TASK_NAME, usTimerTaskStackSize, NULL, ( ( UBaseType_t ) configTIMER_TASK_PRIORITY ) | portPRIVILEGE_BIT, &xTimerTaskHandle, pxTimerTaskStackBuffer, pxTimerTaskTCBBuffer, NULL );
		    HIDE_Workload_StoreTaskHandle(priority_NUM_RTOS_DAEMON_TASK,xTimerTaskHandle);
		}
		#else
		{
			/* Create the timer task without storing its handle. */
			xReturn = xTaskGenericCreate( prvTimerTask, conficTIMER_TASK_NAME, usTimerTaskStackSize, NULL, ( ( UBaseType_t ) configTIMER_TASK_PRIORITY ) | portPRIVILEGE_BIT, NULL, pxTimerTaskStackBuffer, pxTimerTaskTCBBuffer, NULL );
		}
		#endif
	}
	else
